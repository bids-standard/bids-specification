...
| `unique(arg: array) -> array)`                    | The unique values of the input array, retaining their input order.                                                                        | `length(unique(columns.X)) == length(columns.X)`            | True if column `X` contains no duplicate values.                               |
