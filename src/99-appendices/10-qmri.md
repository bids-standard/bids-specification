...
in the specification. Therefore, the  `acq` entity SHOULD be used with `tr1` (lower TR) 
and `tr2` (higher TR) values for this particular use case: 
...
The `acq` entity SHOULD be used with `anat` and `sfamap` (scaled fa map) values 
...
└── sub-01/
     └── fmap/
         ...
         ├── sub-01_acq-sfamap_TB1TFL.nii.gz
         └── sub-01_acq-sfamap_TB1TFL.json
```
...
with identical acquisition parameters. In this respect, the data pair answers 
to the description of the `run` entity.

To properly identify constituents of this particular method, values of the `run`
entity MUST index the images as follows:  
...
└── sub-01/
     └── fmap/
         ├── sub-01_run-1_RB1COR.nii.gz (Body coil)
         ├── sub-01_run-1_RB1COR.json
         ├── sub-01_run-2_RB1COR.nii.gz (Head coil)
         └── sub-01_run-2_RB1COR.json
```
...
```diff
 ...
 └── sub-01/                                                | | | | S
     └── fmap/                                              | | | | Y
         ├── sub-01_acq-PDw_run-1_RB1COR.nii.gz             | | | | M
         ├── sub-01_acq-PDw_run-1_RB1COR.json               | | | | L
         ├── sub-01_acq-PDw_run-2_RB1COR.nii.gz             | | | | I
         ├── sub-01_acq-PDw_run-2_RB1COR.json               | | | | N
