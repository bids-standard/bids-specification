...
If there are split files and the optional `scans.tsv` is provided, remember to list the
files separately in `scans.tsv` and that the entries for the `acq_time` column in `scans.tsv`
