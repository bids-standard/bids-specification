...
A wide variety of motion capture systems are used in human research, resulting in different native data formats. This BIDS extension proposal (BEP) specifically deals any standard outputs from motion capture systems such as positions, orientations or acceleration among others. For camera-based motion capture the raw camera footage falls out of scope. To share motion data recorded using an optical system, typically the position or orientation or likewise system output is formatted in BIDS, not the raw camera footage.
...
| units         | REQUIRED              | string        | Physical or virtual unit of the value represented in this channel, for example, `rad` radian or `deg` degrees for angular quantities or `m` for position data. See the BIDS spec for guidelines for Units and Prefixes. If motion data is recorded in a virtual space and deviate from standard SI units, the unit used MUST be specified in the sidecar `*_motion.json` file (for example `vm` for virtual meters). For reporting orientation angles, please use `rad` for euler angles or `n/a` for quaternions. |   |
