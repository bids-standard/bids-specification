...
1.  A README file SHOULD be found at the root of the `sourcedata` folder 
    (or both). This file should describe the nature of the
    raw data or the derived data. We RECOMMEND including the PDF print-out with 
    the actual sequence parameters generated by the scanner in the `sourcedata` 
    folder.
