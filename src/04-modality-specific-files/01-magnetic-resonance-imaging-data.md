...
|  BolusCutOffTimingSequence                       | OPTIONAL, but REQUIRED if `BolusCutOffFlag` is `true`  | [array][] of [number][]    | Array, specifying the timing after labeling, in seconds, of the start of the bolus cut off saturation pulses. Corresponds to DICOM Tag 0018,925D `ASL Bolus Cut-off Timing Sequence |
