...
| Quantitative susceptibility map (QSM)                   | Chimap | Parametric   | In parts per million (ppm). QSM allows for determining the bulk magnetic susceptibility distribution of 
tissue (Chi) and are usually constructed from the phase images of GRE sequences. The resulting maps are quantitative and are reconstructed by solving the magnetic field to susceptibility source inverse problem ([Wang & Liu, 2014](https://onlinelibrary.wiley.com/doi/10.1002/mrm.25358)). |
