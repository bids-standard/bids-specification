...
<sup>3</sup>We use the "FSL definition", that is the time between the center of the
...
Imaging data acquired during functional imaging (referring to imaging which supports
rapid temporal repetition). This includes but is not limited to task based fMRI
as well as resting state fMRI (meaning that rest is treated as another task). For task
...
the bvecs is with respect to the participant (meaning defined by the axes of the
