...
|  M0               | REQUIRED            | [string][] or [number][] or [boolean][]           | Describes the available M0 information or the location of an `m0scan` file, as either: *Numerical*: a single numerical whole-brain M0 value (e.g. obtained from CSF in another sequence); *SeparateFile*: `sub-<label>[_ses-<label>][_acq-<label>][_rec-<label>][_dir-<label>][_run-<index>]_m0scan.nii[.gz]`; *WithinASL*: *Boolean: `true`* referring to an `m0scan` within the time series (as defined within the `sub-<label>[_ses-<label>][_acq-<label>][_rec-<label>][_run-<index>]_aslcontext.tsv`); *`false`* referring to the use of the (average) `control` image for M0 calibration.    |
