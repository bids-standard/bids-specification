...
def validate_all(bids_dir, regex_schema,
	...
	-----
	* Multi-source validation could be accomplished by distributing the resulting tracking_schema dictionary and further eroding it.
	* The item-wise reporting might need to be removed or parameterized as currently logs of up to 80 MB will be produced.
	"""
...
def write_report(validation_result,
	...
	report_path : str, optional
		A path under which the report is to be saved, the `{}` sting, if included, will be expanded to current datetime, as per the `datetime_format` parameter.
	...
	* Not using f-strings in order to prevent arbitrary code execution.
	...
	with open(report_path, 'w') as f:
		...
		f.write(f'\nSUMMARY:\n{validated_files_count} files were successfully validated.')
