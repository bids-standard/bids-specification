...
bids_contributors_new_emails = pd.read_csv(
    "contributors.tsv", sep="\t", encoding="utf8"
)
bids_contributors_new_emails = rename_columns(bids_contributors_new_emails)
bids_contributors_new_emails_names = [
    x.strip() for x in bids_contributors_new_emails.name
]

print("\n[red]From new email list and not in old email list[/red]")
new_ones = set(bids_contributors_new_emails_names) - set(
    bids_contributors_old_emails_names
)
print(new_ones)
